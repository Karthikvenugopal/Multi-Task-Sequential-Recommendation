"""
Training script for SASRec + PLE (Progressive Layered Extraction).
...
from config import (
    BATCH_SIZE, BEST_PLE_CKPT, CHECKPOINT_DIR, DEVICE,
    EMBED_DIM, DROPOUT, EXPERT_DIM, LEARNING_RATE, MAX_SEQ_LEN,
    MLFLOW_EXPERIMENT, MLFLOW_TRACKING_URI, NUM_BLOCKS, NUM_EPOCHS,
    NUM_HEADS, NUM_SHARED_EXPERTS, NUM_SPECIFIC_EXPERTS,
    PATIENCE, WEIGHT_DECAY, W_CLICK, W_RATING,
    PROCESSED_DATA_PATH,
)
...
def train() -> None:
    ...
    with mlflow.start_run(run_name="ple") as run:
        mlflow.log_params({
            "model": "PLE",
            "d_model": EMBED_DIM,
            "num_heads": NUM_HEADS,
        ...
        best_ndcg = -1.0
        patience_ctr = 0
        resume_path = os.path.join(CHECKPOINT_DIR, "ple_resume.pt")
        start_epoch = 1
        ...
        for epoch in range(start_epoch, NUM_EPOCHS + 1):
            ...
            if ndcg > best_ndcg:
                best_ndcg = ndcg
                patience_ctr = 0
                torch.save(model.state_dict(), BEST_PLE_CKPT)
                print(f"  ✓ New best NDCG@10={ndcg:.4f} — checkpoint saved")
            else:
                patience_ctr += 1
        ...
        model.load_state_dict(torch.load(BEST_PLE_CKPT, map_location=DEVICE))
        ...
        mlflow.log_artifact(BEST_PLE_CKPT, artifact_path="checkpoints")
