"""
Training script for SASRec + MMoE (multi-task, best model).
...
def train() -> None:
    ...
    with mlflow.start_run(run_name="mmoe") as run:
        mlflow.log_params({
            "model": "MMoE",
            "d_model": EMBED_DIM,
            "num_heads": NUM_HEADS,
        ...
        best_ndcg = -1.0
        patience_ctr = 0
        resume_path = os.path.join(CHECKPOINT_DIR, "mmoe_resume.pt")
        start_epoch = 1
        ...
        for epoch in range(start_epoch, NUM_EPOCHS + 1):
            ...
            if ndcg > best_ndcg:
                best_ndcg = ndcg
                patience_ctr = 0
                torch.save(model.state_dict(), BEST_MMOE_CKPT)
        ...
        model.load_state_dict(torch.load(BEST_MMOE_CKPT, map_location=DEVICE))
        ...
        mlflow.log_artifact(BEST_MMOE_CKPT, artifact_path="checkpoints")
